"""Store-root guard for the @victframework/cognee worker (bundled asset).

Originally built for the C2 disposable proof workspace; this copy ships
ALONGSIDE the pack worker (pack/src/worker/cognee_worker.py), which passes
its own explicit --store-root boundary at startup (fail closed without one).
...
storage root into the allowed boundary — for the normal ``.venv``
